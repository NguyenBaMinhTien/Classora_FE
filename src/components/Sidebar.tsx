...
export default function Sidebar() {
  ...
  const menuItems = [
    ...
    { id: 'lecturers', label: 'Giảng viên', icon: Users, path: '/lecturers' },
    { id: 'courses', label: 'Khóa học', icon: BookOpen, path: '/courses' },
    { id: 'schedule', label: 'Lịch học', icon: CalendarDays, path: '/schedule' },
    { id: 'rooms', label: 'Phòng học', icon: DoorOpen, path: '/rooms' },
  ];
            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  {isActive && (
                    <div className="ml-auto w-1 h-5 bg-[#10b77f] rounded-full" />
                  )}
                </>
              )}
            </NavLink>
        ...
        <div className="flex items-center gap-3 px-4 py-2 rounded-xl">
          ...
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <div
              ...
              ${role === 'admin' ? 'bg-red-100' : 'bg-emerald-100'}`}
            >
              <SquareUser
                className={`w-5 h-5 
                ${role === 'admin' ? 'text-red-600' : 'text-emerald-600'}`}
              />
            </div>
